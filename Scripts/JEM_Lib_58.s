...
//Same as JEM_setCondenser//LENS3
void JEM_setBrightness(number x)
{
	LENS3SetCL3(x)
}

//Same as JEM_getCondenser//LENS3
...
void JEM_setShiftXRel(number x)//nm
{
	STAGE3SetXRel(x)
}

void JEM_setShiftYRel(number x)//nm
{
	STAGE3SetYRel(x)
}

void JEM_setShiftZRel(number x)//nm
{
	STAGE3SetZRel(x)
}

void JEM_setTiltAlpha(number x)//deg
{
	STAGE3SetTiltXAngle(x)
}

void JEM_setTiltBeta(number x)//deg
{
...
//Doesn't work in x64 version
void JEM_getFunctionMode(number &n,string &s)
{
	//JEOLCOM_EOS3_GetFunctionMode(n,s)
...
void JEM_getScreenPosition(number &x)
{
	//DETECTOR3GetScreen(x)
}
